...
def write_claim_provenance(root: Path) -> None:
    ...
    for claim_id in range(1, 7):
        ...
        if runtime is None and "wall_seconds" in summary:
            runtime = {
                "wall_seconds": summary["wall_seconds"],
                "cpu_seconds": summary.get("cpu_seconds"),
            }
        if runtime is None and (claim_dir / "independent_checker.json").exists():
            runtime = json.loads(
                (claim_dir / "independent_checker.json").read_text()
            ).get("runtime")
        artifact_hashes = {

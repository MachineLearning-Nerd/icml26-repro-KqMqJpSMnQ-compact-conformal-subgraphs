...
The approved text-only evidence release is published at Hugging Face revision
[`5e743ef78df75e81ddcce5785ee39e16f59a9cad`](https://huggingface.co/spaces/DineshAI/KqMqJpSMnQ/commit/5e743ef78df75e81ddcce5785ee39e16f59a9cad).
It is **awaiting live judge evaluation**. The current judged score remains
...
| --- | --- | --- | --- | --- |
| `main` | Public landing page and presentation artifacts | Not run as an experiment (publication surface) | Published and awaiting live judge; current score remains 5/12 | — |
...
| [`orx/final-evidence-snapshot`](https://github.com/MachineLearning-Nerd/icml26-repro-KqMqJpSMnQ-compact-conformal-subgraphs/tree/orx/final-evidence-snapshot) | Snapshot terminal evidence and the 29-file-preserving text-only Space bundle | `uv run --frozen python repro/src/run_claims.py --out outputs/claims.json && uv run --frozen python -m pytest repro/tests -q && uv run --frozen python repro/src/verify_gate.py` | Passed all six authoritative routes; exposed one contradictory legacy aggregate label | Local CPU |
| [`orx/consistent-final-release-gate`](https://github.com/MachineLearning-Nerd/icml26-repro-KqMqJpSMnQ-compact-conformal-subgraphs/tree/orx/consistent-final-release-gate) | Make aggregate verdict metadata consistent with the six authoritative routes | `uv run --frozen python repro/src/run_claims.py --out outputs/claims.json && uv run --frozen python -m pytest repro/tests -q && uv run --frozen python repro/src/verify_gate.py` | Final gate passed: seven tests, six authoritative verdicts, and all negative controls | Local CPU |
